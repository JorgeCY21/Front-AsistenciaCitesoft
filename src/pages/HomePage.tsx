      ...
      <header className="bg-gradient-to-r from-[#90A23C] to-[#7C8D34] text-white px-4 py-4 shadow-lg">
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <nav className="flex gap-2 w-full sm:w-auto">
            <button
              className={`flex-1 sm:flex-none px-4 py-2 rounded-lg transition-all font-semibold text-sm sm:text-base ${view === 'scanner' ? 'bg-white text-[#90A23C] shadow-md' : 'bg-transparent text-white hover:bg-white/10'}`}
              onClick={() => setView('scanner')}
            >
              Registrar
            </button>
            <button
              className={`flex-1 sm:flex-none px-4 py-2 rounded-lg transition-all font-semibold text-sm sm:text-base ${view === 'history' ? 'bg-white text-[#90A23C] shadow-md' : 'bg-transparent text-white hover:bg-white/10'}`}
      ...
      <main className="max-w-4xl mx-auto p-4 sm:p-6">
        {loading ? (
          <div className="flex justify-center items-center h-64">
            <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-[#90A23C]"></div>
          </div>
        ) : (
          <>
            {view === 'scanner' && (
                    ...
                    {!scanned ? (
                      ...
                      <div className="text-center py-8">
                        ...
                        <button
                          ...
                          className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-[#90A23C] hover:bg-[#7C8D34] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#90A23C]"
                        >
